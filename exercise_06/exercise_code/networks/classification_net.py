...
class MyOwnNetwork(ClassificationNet):
    ...
    def __init__(self, activation=LeakyRelu(), num_layer=3,
                 input_size=3 * 32 * 32, hidden_size=60,
                 std=1e-3, num_classes=10, reg=0, **kwargs):
        """
        ...
        # Initialize random gaussian weights for all layers and bias
        self.num_layer = num_layer
        self.params = {"W1": std*np.random.randn(input_size, hidden_size),
                       'b1': std*np.random.randn(hidden_size)}

        for i in range(num_layer - 2):
            self.params['W' + str(i + 2)] = std*np.random.randn(hidden_size, hidden_size)
            self.params['b' + str(i + 2)] = std*np.random.randn(hidden_size),

        self.params['W' + str(num_layer)] = std*np.random.randn(hidden_size, num_classes)
        self.params['b' + str(num_layer)] = std*np.random.randn(num_classes),
